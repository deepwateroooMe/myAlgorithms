...
// 正好 TreeSet 中提供了这个方法 ceil() ，可以很方便的找出这个元素： 返回大于或等于e的最小元素；如果没有这样的元素，则返回null
        ...
        private void dfs(int idx, int sum, int [] a, int k) {
            ...
            for (int i = 0; i < dp.size(); i++) {
                LinkedList<Integer> cur = dp.get(i);
                int dist = 0;
                if (cur.size() < n / k && cur.peekLast() < a[idx]) { // 每个链表按升序排列的
                    dist = a[idx] - cur.peekLast();
                    cur.addLast(a[idx]);
                    dfs(idx+1, sum + dist, a, k);
                    cur.removeLast();
                }
            }
            if (dp.size() < k) {
    ...
    public static void main(String[] args) {
        ...
        int []  a = new int []  {1, 2, 1, 7};

        int r = s.minimumIncompatibility(a, 2);
