...
public class calculateMinimumHP {
    public static class Solution {
        ...
        public void helper(int [][] bd, Liveegy [][] lmt, int i, int j, int egy, int curr, List<Integer> res) {
            int m = bd.length;
            int n = bd[0].length;
            if (i >= m || j >= n) return;
            if (i == m - 1 && j == n - 1) {
            ...
            if (bd[i][j] >= 0) {
                ...
                if (egy + bd[i][j] >= 0) { // alive
                    egy = egy + bd[i][j];
                } else {
                    egy = 0;
                    curr += (bd[i][j] + egy);
                    if (curr < res.get(0)) return;
                }
            }
            if (curr > lmt[i][j].val && curr + egy >= lmt[i][j].val + lmt[i][j].egy) {
                lmt[i][j].val = curr;
            } else if (curr <= lmt[i][j].val && egy < lmt[i][j].egy) return;
            helper(bd, lmt, i + 1, j, egy, curr, res);
            helper(bd, lmt, i, j + 1, egy, curr, res);
            return;
        }
        ...
        public int calculateMinimumHP0(int[][] dungeon) {
            ...
            res.add(Integer.MIN_VALUE);  // result
            res.add(0);
            res.add(0);
            int m = dungeon.length;
            int n = dungeon[0].length;
            Liveegy [][] limit = new Liveegy[m][n];
            for (int i = 0; i < m; i++) 
                for (int j = 0; j < n; j++) 
                    limit[i][j] = new Liveegy(Integer.MIN_VALUE, 0);
            limit[0][0].val = dungeon[0][0];
            helper(dungeon, limit, 0, 0, 0, 0, res);
        ...
        public int calculateMinimumHP(int[][] dungeon) {
            ...
            for (int i = m - 2; i >= 0; i--) 
                for (int j = n - 2; j >= 0; j--) {
                    int fdown = Math.max(res[i + 1][j] - dungeon[i][j], 1);
                    int fright = Math.max(res[i][j + 1] - dungeon[i][j], 1);
                    res[i][j] = Math.min(fdown, fright);
                }
            return res[0][0];
        }
    }
