...
// 建的是：可以去到哪里
        ...
        // TODO TODO TODO: 硬币可以重复使用，是怎么处理的？
        public int coinChange(int[] a, int t) {
            ...
            for (int i = n-1; i >= 0; i--) { // 只是从大到小，来遍历硬币
                ...
                f[v] = 1;
                for (int j = t; j >= v; j--) 
                    f[j] = Math.min(f[j-v] + 1, f[j % v] + j / v);
            }
        ...
        public int countGoodStrings(int lo, int hi, int zoo, int one) { // TODO TODO TODO: 
            f = new int [hi+1];
            ...
            // dfs(lo-1, 0, zoo, one);
            // return v - f[0];
            return v - dfs(lo-1, 0, zoo, one);
            // return v - dfs(lo, 0, zoo, one);
        }
        static final int mod = (int)1e9 + 7;
       int [] f;
        ...
        public boolean checkValidString(String t) { // TODO TODO TODO: 52/83
            n = t.length();
            s = t.toCharArray();
        ...
        int n;
        Boolean [][][] f;
        Boolean dfs(int idx, int l, int r) {
            // System.out.println("\n idx: " + idx);
            // System.out.println("l: " + l);
            // System.out.println("r: " + r);
            if (idx == n) return l == r;
            // if (f[idx][l][r] != null && f[idx]) return f[idx]; // 没明白这里为什么，奇怪地过了很多测试。。。
            if (f[idx][l][r] != null) return f[idx][l][r];
            // if (r > l) return f[idx][l][r] = false; // 这个修改好像退回去了。。。。。不对！！！
            if (s[idx] == '(') l++;
            else if (s[idx] == ')') r++;
            // '*' 还没有处理
            if (r > l && s[idx] != '*' || s[i] == '*' && r - l > 1) return f[idx][l][r] = false;
            if (s[idx] != '*' && dfs(idx+1, l, r)) return f[idx][l][r] = true;
            if (s[idx] == '*') {
                if (dfs(idx+1, l+1, r)) return f[idx][l][r] = true;
                if (dfs(idx+1, l, r)) return f[idx][l][r] = true;
                if (l >= r+1 && dfs(idx+1, l, r+1)) return f[idx][l][r] = true;
            }
            return f[idx][l][r] = false;
        }
    }  
