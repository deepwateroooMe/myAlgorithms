...
//         }
		...
		// public int maxRepeating(String T, String a) {
		// 	int n = T.length(), m = a.length();
		// 	int r = 0;
		// 	StringBuilder cur = new StringBuilder(a);
		// 	while (T.indexOf(cur.toString()) != -1) {
		// 		r++;
		// 		cur = new StringBuilder(a.repeat(r+1));
		// 	}
		// 	return r;
		// }
        ...
        //     int n = a.length;
        //     int [] f = new int [n];
        //     f[0] = Math.min(Math.min(b[1], b[2]), b[0]);
        //     for (int i = 1; i < n; i++) {
        //         f[i] = f[i-1] + Math.min(Math.min(b[1], b[2]), b[0]); // 按天算，最贵
        //         for (int j = i-1; j >= 0; j--) {
        //             if (a[i] - a[j] <= 6) f[i] = Math.min(f[i], (j == 0 ? 0 : f[j-1]) + b[1]);
        //             if (a[i] - a[j] <= 29) f[i] = Math.min(f[i], (j == 0 ? 0 : f[j-1]) + b[2]);
		// 			else break;
        //         }
        //     }
		...
		// // 亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹，就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！
		// // 亲爱的表哥的活宝妹，以前的脑袋——一年前的脑袋里装的是什么？怎么会这么简单的题目不会写，写了一堆破烂摆那里？
		// // 亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹，就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！
		...
		// 	for (int i = 0; i < n; i++) {
		// 		if (sc.contains(s[i]))
		// 		// r += i+1 + (n-1-i) + (i == 0 || i == n-1 ? 0 : (long)i * (long)(n-1-i)); // 以 i 结尾；以 i 开头； i 在中间
		// 		r += n + (i == 0 || i == n-1 ? 0 : (long)i * (long)(n-1-i)); // 以 i 结尾；以 i 开头； i 在中间
		// 	}
		// 	return r;
		// }

		// // 亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹，就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！
		// // 一遍遍历的话，是说：左边找【 a[i]+i 最大值】，右边找【 a[j]-j 最大值】稀里糊涂就写过了。。。
		// // 亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹，就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！
		// public int maxScoreSightseeingPair(int[] a) {
		// 	int n = a.length, ans = 0;
		// 	Queue<int []> l = new PriorityQueue<>((x, y) -> x[0] != y[0] ? y[0] - x[0] : x[1] - y[1]);
		// 	Queue<int []> r = new PriorityQueue<>((x, y) -> x[0] != y[0] ? y[0] - x[0] : x[1] - y[1]);
		// 	for (int i = 0; i < n; i++) {
		// 		r.offer(new int [] {a[i]-i, i});
		// 		if (!l.isEmpty() && l.peek()[1] < i)
		// 		ans = Math.max(ans, (!r.isEmpty() && r.peek()[1] > l.peek()[1] ? r.peek()[0] : a[i]-i) + l.peek()[0]);
		// 		l.offer(new int [] {a[i]+i, i});
		// 	}
		// 	return ans;
		// }
        ...
        //     if (i > n-3) return f[i] = 1 + dfs(i+1); //i==n-2 
        //     int r = Integer.MAX_VALUE / 2, j = 0;
        //     for (j = n; j >= i+1; j--) {
		// 		// 这里，题目意思，可能没理解正确：5 的冥！亲爱的表哥的活宝妹，是活宝妹的亲爱的表哥的冥！！
		// 		// 亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹，就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！
        //         if (j - i >= 3 && t.substring(j-3, j).equals("101") || j - i >= 4 && t.substring(j-4, j).equals("1010")) {
		...
		// 亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹，就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！ 2927=2925
		// 亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹，就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！ 2927=2925
		public long maximumScoreAfterOperations(int [][] a, int[] b) {
			int n = a.length;
			g = new ArrayList[n];
    ...
    public static void main (String[] args) { // 【爱表哥，爱生活！！！活宝妹就是一定要嫁给亲爱的表哥！！！】
		...
		String a = "100111000110111";

        int r = s.minimumBeautifulSubstrings(a);
