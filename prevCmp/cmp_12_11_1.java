...
public class cmp {
    public static class Solution {
        ...
        public List<Integer> goodDaysToRobBank(int[] a, int t) {
                ...
                // for (int i = 0; i < n-1; i++) { // 第二版：笨版，把它们换成动规的行式避免重复计算
                //     for ( j = i-1; j >= 0; j--)
                //         if (a[j] < a[j+1]) break;
                //     dn[i] = i - j-1;
                // }
        ...
        //     }
        // }
        public int maximumDetonation(int[][] a) { // 做题的时候，需要把思路理得再透彻一些
            ...
            if (maxCnt == n-1) return n; // 这个提前检测居然还能提高不少效能？！！！
            vis = new boolean [n+1];
            for (int i = 0; i < n; i++) {
    ...
    TreeSet<Location> locations = new TreeSet<>(); // 参考网上，另一种treeset的写法
    Location current = null; // last returned index, 这里这个元素就成为这个设计维护的重点
    public void add (String name, int score) {
        ...
        if (current != null && location.compareTo(current) < 0) 
            current = locations.lower(current);
    }
    public String get() {
        if (current == null) { // first call
            current = locations.first();
        } else { // move the pointer
            current = locations.higher(current);
        }
        return current.name;
    }
    static class Location implements Comparable<Location> {
        final String name;
        final int score;
        public Location(String n, int s) {
            name = n;
            score = s;
        }
        public int compareTo(Location l) {
            return score == l.score ? name.compareTo(l.name) : -Integer.compare(score, l.score);
        }
    }
    }
