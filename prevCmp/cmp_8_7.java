...
// 		}
	...
	// }
		...
		// // 【并查集】的思路，昨天晚上、在、坐在闹哄哄的马路边、脑袋昏昏两耳朵塞耳塞的亲爱的表哥的活宝妹，这个思路曾一闪而过、、
		// // 可是最终它不入亲爱的表哥的活宝妹的脑袋，没能想到想透。。
		// public int[] shortestDistanceAfterQueries(int n, int [][] qs) { 
		...
		// }
		// // 【亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹，就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
		// // 亲爱的表哥的活宝妹，先去把那个给弄了。。然后再回来，把这个题目吃透，能弄得明白
		// // 死难死难的2 个题目。。破烂题解，讲解、也说不清楚。。
		// class UnionFind {
		// 	int [] f;
		...
		// 			f[i] = i;
		// 	}
		// 	public int find(int i) { // 【TODO】：感觉这里，亲爱的表哥的活宝妹，并没弄明白。。
		// 		if (f[i] != i)
		// 			f[i] = find(f[i]);
		// 		return f[i];
		// 	}
		// 	public void union(int i, int j) {
		// 		if (find(i) != find(j)) // 觉得，这里，把它2 个都更新到精确了
		// 			f[i] = f[j]; // 下面，直接赋值、合并，就可以了
		// 	}
		// }
		// // 【亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹，就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
		// public int[] shortestDistanceAfterQueries(int n, int [][] qs) { // 【TODO】：这里写得【神仙跳】看不懂。。
		// 	int m = qs.length, cnts = n-1;
		// 	// 用并查集实现边的合并。初始化一个大小为 n−1 的并查集，并查集中的节点 i 表示题目的边 i→(i+1)。
		// 	// 连一条从 L 到 R 的边，相当于把并查集中的节点 L,L+1,L+2⋯,R−2 合并到并查集中的节点 R−1 上。
		// 	UnionFind uf = new UnionFind(n-1); // 【并查集】：处理，最多 n-1 条边的、合并关系。可以把这里的处理元素【边】，理解想成是，原始边两个端点的【左端点】
		// 	int [] ans = new int [m];
		// 	for (int i = 0; i < m; i++) {
		// 		// l,r: 是边的两个端点 
		// 		int l = qs[i][0], r = qs[i][1] - 1, p = uf.find(r); // p: 并查集里，现在【右端点】所在的组群
		// 		// for (int j = l; j < r; j++) { // 遍历：要合并的所有【边】节点：【L,R-1】
		// 		// 注意每个点只会被合并一次，在后面的循环中会被 i = find(l) 以及 i = find(i + 1) 跳过。由于数组的特殊性，每次合并的复杂度为 O(1) 【TODO】：看不懂！！
		// 		for (int j = uf.find(l); j < r; j = uf.find(j + 1)) { // 【TODO】：这里写得【神仙跳】看不懂。。
		// 			// 合并【l,r】，更新到一个组合里
		// 			uf.f[j] = p;
		// 			--cnts;
		// 		}
...
// 亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹就是一定要,一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！
    public static void main (String[] args) {
		...
		int [][] a = new int [][] {{6,10},{1,12},{7,9}};

		int [] r = s.shortestDistanceAfterQueries(13, a);
		System.out.println(Arrays.toString(r));
    }
}
