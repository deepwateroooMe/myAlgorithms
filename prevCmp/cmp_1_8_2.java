...
public class cmp {
    public static class Solution {
        ...
        public int earliestFullBloom(int[] pt, int[] gt) {
            ...
            for (int i = 0; i < n; i++) idx.add(i); // 两行代码效果一致，但运行速度应该是 comment的快，lambda需要运行时生成太多类了
            Collections.sort(idx, Comparator.<Integer, Integer>comparing(i -> -gt[i]).thenComparing(i -> -pt[i]));
            // Collections.sort(idx, (i, j) -> gt[i] != gt[j] ? gt[j] - gt[i] : pt[j] - pt[i]);
            int max = 0;
            for (int i = 0, psum = 0; i < n; i++) {
                int j = idx.get(i);
                int t = gt[j] + pt[j] + psum;
                max = Math.max(max, t);
                psum += pt[j]; // 种植的时间必须每一个都累加，没有任何优化的余地
            }
            return max;
        }
    }
