...
public class cmp {
    public static class Solution {
        ...
        //         }
        //     }
        //     // 这个最开始的思路差了一步，虽然压缩成26个类，但如果一个字母的出现次数过多，频繁出入椎的代价也是很大的
        //     // 所以缺少了，既出堆就再不入堆，把它存在栈上，直到取完为止这样一个便捷操作
        ...
        //     return ans;
        // }
        // public String repeatLimitedString(String t, int lt) { // bug 144/150 passed, leetcode is too bad don't show wrong test case to users......
        ...
        // public long countPairs(int[] a, int k) {    // tle： 92/115 passed: 在想这个题目是否还含分解质因子、动态规划之类的？为什么会超时呢？
        ...
        //             recnt++;
        //             cnt += n-1;
        //             // m.put(a[i], m.getOrDefault(a[i], 0) + 1); // 这种不用加进去
        //             continue;
        //         }
        ...
        // All we need to do is to count/group the numbers by the gcds.
        // And if gcd(num[i], k) * gcd(num[j], k) is divisible by k , num[i] * num[j] should also be divisible.
        public long countPairs(int[] a, int k) {   
            long n = a.length, cnt = 0;
            Map<Integer, Integer> gcd = new HashMap<>();
            for (int i = 0; i < n; i++) {
                int cur = getGcd(a[i], k);
                for (Integer key : gcd.keySet()) {
                    if ((long)cur * key % k == 0) {
                        cnt += gcd.get(key);
                    }
                }
                gcd.put(cur, gcd.getOrDefault(cur, 0) + 1);
            }
            return cnt;
        }
