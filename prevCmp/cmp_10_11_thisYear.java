...
//         } 
        // // 【亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹，就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
        // // 亲爱的表哥的活宝妹，今天下午，再看昨天晚上这个题目，狠简单了。。。【TODO】：仍然有一个【不稳定】的【BUG】有时候会出现
        ...
        //     return ans;
        // }
        // // 【TODO】：上面的【自平衡树解法】：【-1,1】0,0 还有个什么狗屁破烂 seed 自平衡、结果不稳定的问题？改天再写，现在找不出这个【BUG：】了
        // // 【亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹，就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
        // // 亲爱的表哥的活宝妹，把这个【动态添加节点】终于写出来了，可是感觉、这种写法【不经典】不够好，希望有个【经典的、典型、模块】可以【时时、处处、想用就能、套用】！！！
        // // 【TODO】亲爱的表哥的活宝妹，觉得：这个【动态添加节点】的狗屁题解，写得不好，太笨！！【动态添加节点】可以同时打卡【懒惰标记】吗？还没想明白，再多写几个
        ...
        //         return 0;
        //     // 当，亲爱的表哥的活宝妹，笨宝妹，把【动态添加节点、线段树】的【管辖区】设置得无限大【Long.MIN_VALUE, Long.MAX_VALUE】，这里永远返回树大小：【树根 r.rr】
        ...
        // 亲爱的表哥的活宝妹，【TODO】：下午再来写，这个【区间加区间和】等【区间】相关，写懂写会、理解透彻！！！
        // public class BIT {
        ...
        List<Integer> find_occurrences(String s, String p) {
            String cur = p + "#" + s;
            int m = p.length(), n = s.length();
            List<Integer> li = new ArrayList<>();
            int [] f = prefix_function(cur);
            // 从【合并串 cur】中S 子串的、【开始下标 m+1】开始遍历
            for (int i = m+1; i < n+m+1; i++) {
                if (f[i] == m) // 最长【前缀长度】与【标准串 p】长度一样：最大可能长度
                    // i-(m-1): 以 i 结尾，长度为 m 的最长相同后缀，后缀开始的下标
                    // i - (m-1) - (m+1): 以 i 结尾，长度为 m 的最长相同后缀，后缀开始的下标，所对应的 S 串中，原始下标
                    // li.add(i - (m-1) - (m+1));
                    li.add(i - 2 * m);
            }
            return li;
        ...
        // 【亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹，就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
        // 统计每个前缀的出现次数：【TODO】：这个题解，亲爱的表哥的活宝妹，没有看懂，要借助 debug 弄明白
        // 【第一变种问题】：给定一个长度为 n 的字符串 s，希望统计每个前缀 s[0:i] 在同一个字符串的出现次数，
        int [] prefix_function(String t) { 
            int n = t.length(); char [] s = t.toCharArray();
            int [] f = new int [n];
            for (int i = 1; i < n; i++) {
        ...
        int [] getCnts(String t) {
            int n = t.length();
            // 考虑位置 i 的前缀函数值 f[i]。
            // 根据定义，其意味着字符串 s 一个长度为 f[i] 的前缀在位置 i 出现并以 i 为右端点，同时不存在一个更长的前缀满足前述定义。
            // 与此同时，更短的前缀可能以该位置为右端点。
            int [] f = prefix_function(t);
            int [] r = new int [n+1]; // 答案： 
            for (int v : f) // 答案：下标对应，最长长度
                r[v]++; // s[0:i] 最长长度：出现频率
            // 【TODO】亲爱的表哥的活宝妹，不明白，为什么是倒序遍历，加到【更小长度】的计数上，而不是加在【更长长度】的计数上？
            // 因为，【较小长度】的计数，永远大于，【较大长度】的计数。
            // 答案 r[1:N] 【自前往后】遍历：长度【自小变大】，答案计数【自大变小】！！
            for (int i = n-1; i >= 0; i--) // 【TODO】：这里，亲爱的表哥的活宝妹没读懂，为什么是，倒序遍历的？没理解
                // 我们遇到了在计算【前缀函数】时已经回答过的问题：
                // 给定一个【长度为 j=f[i]】的【前缀】，同时其也是一个【右端点位于 i】 的【后缀】，下一个【更小的、前缀长度 k < (j=f[i])】是多少？
                // 【该长度的前缀】，需同时也是一个: 【右端点为 i】的【后缀】。
                // 亲爱的表哥的活宝妹的、【推理方法】是：匹配完 s[i]=s[f[i-1]] 后，倒序往前推，去匹配 s[i-1] = s[ f[i-1] - 1]
                // 亲爱的表哥的活宝妹的、【推理方法】是：更小、缩短缩进了一层的长度，会是，当前【右端点 i】在【下标？长度？为 f[i-1]】为右端点时的、最长长度，即
                // 即：int k = f[f[i-1] - 1]; s[0:k] = s[(f[i-1]-1)-(k-1):k] 的、【当前右端点 i】在【右端点为 f[i-1]-1】时，【最长长度 k = f[f[i-1]-1]】
                // 亲爱的表哥的活宝妹，也不知道，这么想，对不对。。。【TODO】没事儿的时候，多想一想。等想明白，就能确认
                // 因此【以位置 i 为右端点】，有长度为 f[i] 的前缀，有长度为 f[f[i] - 1] 的前缀，有长度为 f[f[f[i] - 1] - 1] 的前缀，等等，直到长度变为 0。
                // 答案是：k = f[j-1]
                r[f[i-1]] += r[i];
            // 遍历 s[0:i] 所有、可能长度： s.substring(0,i+1) 原串的、计数1, 累加到答案里
            for (int i = 1; i <= n; i++)
                r[i]++;
            return r;
        } 
...
// 亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹就是一定要,一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！ 
    public static void main (String[] args) { 
        ...
        int []  a = new int []  {5, 6, 4, 5};
        int [][] b = new int [][] {{2,3,9},{1,0,2},{1,3,3}};
        ...
        List<Integer> r = s.countOfPeaks(a, b);
