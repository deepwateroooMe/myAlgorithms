...
void findSum(TreeNode* root, int sum, vector<int> v, vector< vector<int> > &res) {
    if (!root) return;
    ...
    findSum(root->left, sum, v1, res);
    findSum(root->right, sum, v2, res);
}
