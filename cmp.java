...
//         }
        ...
        // 实在是不喜欢这个题，太恶心人了
        public int distMoney(int v, int n) {
            int r = 0, idx = 0;
            if (v < n) return 0;
            v -= n; // 每人先分 1 块钱
            if (v == 0) return 0;
            // if (v < 7) return -1; // 这里要区分什么时候不能分，什么时候分的是 0
            while (idx < n-1 && v > 0) {
                if (v >= 7) {
                    r ++;
                    v -= 7;
