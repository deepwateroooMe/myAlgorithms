...
//         }
        ...
        // public boolean isAcronym(List<String> l, String t) {
        //     int m = l.size(), n = t.length(); char [] s = t.toCharArray();
        //     if (m != n) return false;
        //     for (int i = 0; i < n; i++)
        //         if (s[i] != l.get(i).charAt(0)) return false;
        //     return true;
        // }
        ...
        // // 【最大值线段树】：忘光了吧。。。??? 现在知道这个题目可以动规，细节可能没想透彻，等会儿再写，。。。293/534
        // // 亲爱的表哥的活宝妹，现在总是有极大的兴趣，来挑战最后一个难题。。。【爱表哥，爱生活！！！任何时候，亲爱的表哥的活宝妹就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
        // public int maximizeTheProfit(int n, List<List<Integer>> l) {
        //     // 把链表排序：因为顺序不影响结果: [ 起点，终点，利润 ] 升升降，排列
        //     Collections.sort(l, (x, y) -> x.get(0) != y.get(0) ? x.get(0) - y.get(0) : (x.get(1) != y.get(1) ? x.get(1) - y.get(1) : y.get(2) - x.get(2)));
        //     System.out.println("l.size(): " + l.size());
        //     for (int z = 0; z < l.size(); ++z) 
        //         System.out.println(Arrays.toString(l.get(z).toArray()));

        //     int m = l.size(), max = 0;
        //     int [] f = new int [n]; // 到某个终点：最大利润 
        //     for (int i = 0; i < m; i++) {
        //         List<Integer> li = l.get(i);
        //         int ll = li.get(0), rr = li.get(1), v = li.get(2);
        //         if (i == 0) {
        //             f[rr] = v;
        //             continue;
        //         }
        //         int j = i-1;
        //         // 下面，左右相等时，过滤掉了一些解。。
        //         // while (j >= 0 && l.get(j).get(1) >= ll) j--;
        //         while (j >= 0 && (ll != rr && l.get(j).get(1) >= ll || ll == rr && l.get(j).get(1) > ll)) j--;
        //         if (j < 0) f[rr] = Math.max(v, ll == 0 ? 0 : f[ll-1]); // 当前片段，与先前的片段全有交集，要么买当前片段，要么不要当前片段
        //         else if (j >= 0 && ll == rr && l.get(j).get(1) == ll) f[rr] = Math.max(f[rr], v); // 感觉还要加前面的片段才对，这里。。。。
        //         else f[rr] = Math.max(v + f[l.get(j).get(1)], ll == 0 ? 0 : f[ll-1]); // 当前片段，在中间，【要当前的】＋可以接前一片，不要当前的
        //         max = Math.max(max, f[rr]);
        //     }
        //     return max;
        // }

        // 感觉，像是，双指针，滑动窗口的题目，从左往右遍历: 不知道细节上哪里写错了，过了大半。。。
        public int longestEqualSubarray(List<Integer> li, int k) {
            int n = li.size(), i = 1, cnt = 0; // cnt: 中间其它数的个数计数
            int ans = 1, l = 0, m = 0; // 右是 i 
            while (i < n) { // 右端点：到尾巴
                i = l + 1;
                while (i < n && li.get(i) == li.get(l)) i++;
                if (i == n) return Math.max(ans, i - l - cnt);
                cnt = 0;
                m = -1;
                while (i < n && cnt < k) {
                    if (li.get(i) != li.get(l)) {
                        if (m == -1) m = i; // 记：第一个，不一样的数，过会儿 l 转这里来。。
                        cnt++;
                    }
                    i++;
                }
                System.out.println("cnt: " + cnt);
                if (i == n) {
                    // return Math.max(ans, i - l - cnt); // 这里不能返回，没遍历完。。
                    ans = Math.max(ans, i - l - cnt);
                    l = (m != -1 ? m : i); // 头：从那个不一样的数开始，再遍历
                    i = l + 1;
                    cnt = 0; // 它要清成空 0
                    continue;
                } else { // 剩余，就是用完了 k 个数, 但是，还有一种可能：后面接的，仍是 li.get(l) 相同的数
                    while (i < n && li.get(i) == li.get(l)) i++;
                    ans = Math.max(ans, i - l - cnt);
                    System.out.println("ans: " + ans);
                    l = (m != -1 ? m : i); // 头：从那个不一样的数开始，再遍历
            ...
            return Math.max(ans, i - l - cnt);
        }
     }             
    public static void main (String[] args) { 
        Solution s = new Solution ();
        // int [][] a = new int [][] {{0,0,1},{0,2,2},{1,3,2}};
        // int [][] a = new int [][] {{0,0,5},{3,3,1},{1,2,5},{0,0,7}};
        // int [][] a = new int [][] {{1,3,9},{0,2,10},{1,3,3},{0,1,3},{2,2,1},{3,3,7},{2,2,1},{1,2,9},{1,2,8}};
        // List<List<Integer>> ll = new ArrayList<>();
        // for (int [] v : a) 
        //     ll.add(Arrays.stream(v).boxed().collect(Collectors.toList()));
        // int r = s.maximizeTheProfit(4, ll);
        int [] a = new int [] {919, 273, 843, 858, 49, 79, 581, 45, 204, 110, 818, 446, 745, 373, 854, 9, 544, 912, 57, 222, 450, 678, 54, 580, 37, 945, 311, 461, 293, 398, 851, 548, 643, 565, 673, 363, 505, 958, 217, 928, 874, 812, 90, 225, 77, 662, 555, 222, 318, 874, 495, 56, 353, 29, 580, 985, 792, 73, 930, 256, 644, 59, 454, 879, 604, 602, 10, 291, 680, 166, 468, 911, 60, 374, 883, 844, 748, 961, 784, 276, 974, 306, 46, 57, 855, 202, 794, 966, 778, 277, 628, 285, 171, 943, 949, 135, 91, 23, 772, 436, 870, 506, 590, 121, 922, 695, 815, 124, 192, 15, 952, 77, 738, 678, 299, 303, 838, 966, 709, 623, 990, 406, 160, 697, 189, 271, 46, 446, 676, 305, 276, 492, 238, 83, 919, 466, 537, 485, 859, 124, 424, 281, 786, 81, 685, 806, 63, 649, 511, 601, 712, 201, 423, 910, 993, 618, 791, 393, 538, 753, 966, 348, 247, 352, 230, 408, 550, 866, 958, 376, 60, 889, 34, 957, 972, 391, 75, 937, 402, 140, 102, 861, 725, 136, 379, 687, 859, 238, 752, 240, 647, 736, 690, 610, 539, 715, 518, 434, 560, 945, 143, 988, 873, 5, 751, 607, 494, 344, 469, 580, 489, 741, 771, 522, 576, 219, 486, 49, 496, 62, 922, 466, 999, 162, 353, 665, 544, 894, 154, 167, 296, 23, 519, 495, 476, 581, 91, 838, 295, 407, 413, 851, 211, 160, 583, 173, 625, 659, 985, 248, 888, 544, 263, 81, 53, 28, 401, 953, 278, 34, 994, 983, 847, 623, 951, 124, 591, 436, 530, 964, 848, 225, 454, 510, 446, 171, 525, 941, 395, 525, 484, 672, 307, 981, 79, 950, 163, 488, 516, 800, 968, 780, 833, 328, 606, 925, 459, 426, 671, 823, 577, 205, 999, 346, 370, 592, 788, 317, 320, 672, 300, 103, 401, 229, 712, 410, 466, 406, 46, 706, 40, 639, 66, 65, 436, 763, 137, 539, 134, 890, 545, 175, 100, 658, 254, 844, 515, 672, 775, 158, 879, 586, 133, 107, 214, 804, 895, 590, 376, 321, 420, 428, 367, 65, 701, 212, 487, 20, 236, 759, 169, 264, 806, 922, 13, 635, 791, 985, 262, 953, 363, 547, 875, 169, 689, 712, 220, 769, 31, 505, 657, 500, 504, 942, 392, 118, 273, 487, 565, 193, 318, 687, 654, 435, 238, 141, 668, 945, 521, 610, 777, 263, 11, 832, 877, 121, 901, 973, 973, 182, 264, 957, 996, 966, 259, 187, 520, 843, 825, 985, 222, 856, 658, 323, 637, 605, 326, 65, 211, 845, 224, 737, 226, 363, 841, 165, 962, 348, 274, 872, 504, 97, 800, 322, 419, 182, 140, 99, 523, 951, 695, 623, 388, 509, 301, 419, 612, 772, 265,