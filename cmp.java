...
//         }
        ...
        //     return min;
        // }
        public int minimumCost(int[] s, int[] t, int[][] egs) { // 这个题目已经算是比较透了，改天再写
            n = (int)1e5; si = s[0]; sj = s[1]; ti = t[0]; tj = t[1];
            m = new HashMap<>(); // 建个【有向图】
            bgn = si * n + sj; end = ti * n + tj;
            m.put(bgn, new ArrayList<long []>());
            Arrays.sort(egs, (x, y)-> x[0] != y[0] ? y[0] - x[0] : y[1] - x[1]); // 为的是倒着往回遍历 
            Set<Long> st = new HashSet<>(); // points of tails 因为要添加边，把数组排个序
            for (int [] e : egs) { // 【BUG:】这些边还不够多，边与边之间有可能的话，也要连起来。。。妈的。。。
                int i = e[0], j = e[1], x = e[2], y = e[3], w = e[4];
                int d = Math.abs(i - si) + Math.abs(j - sj);
                long v = i * n + j, u = x * n + y;
                m.get(bgn).add(new long [] {v, (long)d}); // 起点到这里的边
                m.computeIfAbsent(v, z -> new ArrayList<>()).add(new long [] {u, w}); // 特殊路边
                m.computeIfAbsent(u, z -> new ArrayList<>()).add(new long [] {end, Math.abs(x - ti) + Math.abs(y - tj)});
                dm.put(u, Integer.MAX_VALUE);
                dm.put(v, Integer.MAX_VALUE);
                for (long vv : st) { // 这么着，边还是没构建够吗？
                    int ii = (int)(vv / n), jj = (int)(vv % n);
                    if (x <= ii && y <= jj)
                        m.get(u).add(new long [] {vv, Math.abs(x - ii) + Math.abs(y - jj)});
                    dm.put(vv, Integer.MAX_VALUE);
                }
                st.add(v);
            }
            m.get(bgn).add(new long [] {end, (long)(Math.abs(si - ti) + Math.abs(sj - tj))});
            dm.put(end, Integer.MAX_VALUE);
            dm.put(bgn, 0);
            calculateShortestDist();
            return (int)dm.get(end);
        }
        Map<Long, List<long []>> m;
        Map<Long, Integer> dm = new HashMap<>(); // dist map
        int n, si, sj, ti, tj;
        long bgn, end;
       int calculateShortestDist() {
            Queue<long []> q = new PriorityQueue<>((x, y) -> Long.compare(x[1], y[1]));
            q.offer(new long [] {bgn, 0});
            while (!q.isEmpty()) {
                long [] cur = q.poll();
                if (cur[0] == end) return (int)cur[1];
                int i = (int)(cur[0] / n), j = (int)(cur[0] % n), w = (int)cur[1];
                for (long [] r : m.get(cur[0])) {
                    int x = (int)(r[0] / n), y = (int)(r[0] % n), d = (int)r[1];
                    int dt = Math.abs(i - x) + Math.abs(j - y) ;
                    if (w + dt < dm.get(r[0])) {
                        dm.put(r[0], (w + dt));
                        q.offer(new long [] {r[0], (w + dt)});
                    }  
                }
            }
            return -1;
