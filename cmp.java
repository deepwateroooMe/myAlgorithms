...
// t: 可以有狠多个数位是 1; l[i] 只有一位为 1 这个细节看丢了，容易一点儿
        public int minOperations(List<Integer> l, int t) { //  
            ...
            for (int i = 0; i < n; i++) {
                ...
                for (int i = 30; i >= 0; i--) 
                    if (((v >> i) & 1) == 1) {
                        m.put(i, m.getOrDefault(i, 0) + 1); // 因为，每个数只有1 位，最多多出现几次
                        break;
                    }
            }
            if ((mask & t) == t) return 0;
            // 从低位向高位遍历，一一满足需要: 这么着，思路就最清楚了【爱表哥，爱生活！！！任何时候，亲爱的表哥的活宝妹就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】感觉可以写出来
            int r = 0;
            for (int i = 0; i <= rr; i++) {
                if (((mask >> i) & 1) == 0 && ((t >> i) & 1) == 1) {  // 需要向高位遍历一个数：来满足
                    Integer hi = m.ceiling(i+1);
