...
//         }
    ...
    // }
        ...
        int m, n;
        Integer [][] f; 
// i 是遍历的数位，j: 前一位数位的值, 。从高位往低位遍历，可是要比较数的长度, 感觉状态欠缺，需要 boolean;
        // mid: 标记第1 位数字，是否比s[0] 大, 是否比t[0] 小；范围外的只能数位长开区间（m,n）两边不包含
        int dfs(int i, int j, boolean lo, boolean hi, boolean slo, boolean mid) { // 先把这个方法：slo，mid 参数补齐： 0 位必备标记
            System.out.println("\n i: " + i);
            System.out.println("j: " + j);
...
// 不合法的：这里直接返回了，可能?转下面去才能真正确保正确性. 这里也要：这里是最先、最大范围的栽枝；但只是粗裁不够细，仍有假阳性。。。
            if (i > 0 && (lo && i < m && s[i]-'0' > j+1 || hi && i < n && t[i]-'0' < j-1)) return 0; 
            // 因为 0 位扩充了，这里数多了
            // if (i == n) return 1; // 数到了这个长度，可是过程中的呢？（m,n）长度的？
            if (i == n && (m == n || m+1 < n && mid)) return 1; // 数到了这两个端点长度【m,n】，可是过程中的呢？（m,n）长度的？过程中长度，下面的过程中计数了
            ...
            if (i == 0) { // 起始位特殊：可以任意打头，其它受限
                ...
                // i==m 怎么就保证一定比小数,等大更大了？把mid 折成两个变量：比 s[0] 大，比 t[0] 小！！
                if (m+1 < n && (i == m-1 && slo || i >= m && i < n-1)) r += 1; // 数，当前长度（m,n）开区间两者之间，的当前数结果. i=n-1 结果已经数过
// 这两种情况了限制条件：可能需要自上面，转移到这里，确保【不比小的小，不比大的大】                ！！
                // if (!hi)  // 不是最大值，才可以再加：这两个限制条件，好像还是想错了，加多余了？毕竟它处理的是当前数位 i, 与先前位，可以看作关系不大
                //     r = (r + dfs(i+1, j+1, lo && j+1 == s[i] - '0', false, slo, mid)) % mod;
                if (!hi || hi && t[i]-'0' >= j+1) // 当前位取 j+1, 确保合法正确
                    r = (r + dfs(i+1, j+1, lo && j+1 == s[i] - '0', hi && j+1 == t[i]-'0', slo, mid)) % mod;
                if (!lo || lo && s[i]-'0' >= j-1) // 当前位取 j-1, 确保不小于小的数
