...
// 		// 还应该怎么优化呢？ 16.4% 的通过率，说明这个题目确实琐碎。。。折磨死人不偿命！！！亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！
...
// 		}
		...
		// // 亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！
		// // 感觉不知道这个死题目，说的是什么意思，是要倒过来，可是没有想法，不知道怎么去想这个破题目！！
		// public int minimumTimeToInitialState(String S, int k) {
		// 	int n = S.length(); char [] s = S.toCharArray();
		// }

// // 亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！
// 		// 【动规】：数组上的动规. 数组上的求和；以及邻接值的【合法性】比较？
// 		// 感觉这个题目，想得狠结巴、不透彻，今天不写了，改天再写
// 		public int[][] resultGrid(int[][] a, int t) {
// 			int m = a.length, n = a[0].length;
// 			int [][] r = new int [m+1][n+1]; // 求和数组
// 			for (int i = 1; i <= m; i++) 
// 				for (int j = 1; j <= n; j++)
// 					r[i][j] = r[i-1][j] + r[i][j-1] + a[i-1][j-1] - r[i-1][j-1];
// 			int [][] ans = new int [m][n];
// 			// 每个小区：【合法性检测】、仍然应该是动规，自顶向下，自左上角，向右下角
// 			// 改动原数组，不能实现；再分配一个专用数组：用 -1 标记，此格小区不合法
// 			int [][] f = new int [m][n];
// 			for (int i = 0; i < 2; i++) 
// 				for (int j = 1; j < 2; j++) {
// 					if (Math.abs(a[i][j] - a[i][j-1]) > t) {
// 						f[i][j] = 1;
// 						f[i][j-1] = 1;
// 					}
// 					if (i > 0 && Math.abs(a[i][j] - a[i-1][j]) > t) {
// 						f[i][j] = 1;
// 						f[i-1][j] = 1;
// 					}
// 				}
// 			int [][] c = new int [m][n]; // cnter array: how many regions a cell could be valid?
// 			int [][] s = new int [m][n]; // sum array: accumlative current averaged sum ? (avg1 + avg 2...)
// 			int cur = 0;
// 			for (int i = 2; i < m; i++)
// 				for (int j = 2; j < n; j++) {
...
// 					else {
// 						c[i][j]++;
// 						cur = Math.floor((r[i+1][j+1] - r[i][j-2] - r[i-2][j] + r[i-2][j-2]) / 9);
// 						s[i][j] += cur;
// 					}
// 				}
// 		}
...
// 			int m = a.length, n = a[0].length, max = -1;
// 			int [][] f = new int [m][n];
// 			for (int j = 0; j < n; j++) {
// 				max = -1;
// 				for (int i = 0; i < m; i++) {
...
// 		}
 ...
 // // 亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！
 // 		// 亲爱的表哥的活宝妹，这里想不清楚呀：不知道说的是什么意思，感觉，要用个什么【差分数组】：亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！
 // 		// 数据规模小，亲爱的表哥的活宝妹，就这里瞎瓣！！
 // 		public int countMatchingSubarrays(int[] a, int[] p) {
 // 			int m = p.length, n = a.length, sum = Arrays.stream(p).sum(), r = 0;
 // 			int [] f = new int [n], s = new int [n];
 // 			for (int i = 1; i < n; i++) {
 // 				f[i] = (a[i] == a[i-1] ? 0 : (a[i] > a[i-1] ? 1 : -1));
 // 				s[i] = s[i-1] + f[i];// 前缀和
 // 			}
 // 			// System.out.println(Arrays.toString(a));
 ...
 // 			boolean valid = true;
 // 			for (int i = 0; i+m < n; i++) {
 // 				if (s[i+m] - (i == 0 ? 0 : s[i]) != sum) continue;
 // 				System.out.println("\n i: " + i);
 // 				valid = true;
 // 				for (int j = 0; j < m; j++) 
 // 					if (p[j] != f[i+j+1]) {
 ...
 // 		}
		...
		// 		// 亲爱的表哥的活宝妹，这里想不清楚呀：不知道说的是什么意思，感觉，要用个什么【差分数组】：亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！
		// 		// 数据规模小，亲爱的表哥的活宝妹，就这里瞎瓣！！
		// // 我觉得，现在是就是上次的那个O(N) 的一遍扫！上次写了一半没写完，这次一个【－1/0/1】数组里扫 p 片段子数组，应该能够更简单才对，只比一个数的频次
		// 		public int countMatchingSubarrays(int[] a, int[] p) {
		// 			int m = p.length, n = a.length, sum = Arrays.stream(p).sum(), r = 0, cnt = 0, cntp = 0;
		// 			int [] f = new int [n], s = new int [n];
		// 			// 下面想得不对：【和相同】【正数个数相同】不能保证序列完全相同。。。这个细节，写的时候，思路没想对！！
		// 			for (int i = 0; i < m; i++)
		// 				if (sum >= 0 && p[i] > 0 || sum < 0 && p[i] < 0) cntp++;
		// 			for (int i = 1; i < n; i++) {
		// 				f[i] = (a[i] == a[i-1] ? 0 : (a[i] > a[i-1] ? 1 : -1));
		// 				s[i] = s[i-1] + f[i];// 前缀和
		// 				if (sum >= 0 && f[i] == 1 || sum < 0 && f[i] == -1) cnt++; // 数个数最少的非 0 个数
		// 				f[i] = cnt;
		// 			}
		// 			System.out.println(Arrays.toString(a));
		// 			System.out.println(Arrays.toString(s));
		...
		// 数据规模还是不大，亲爱的表哥的活宝妹，还要继续瞎瓣吗？可是，这种破题目，瓣得狠没意思，不写了，改天再写！！
		// // 亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！
		// public int maxPalindromesAfterOperations(String[] S) {
		// 	int n = S.length;
		// 	return 0;
		// }
		...
		// 	int n = a.length, r = 0, x, y;
		// 	for (int i = 0; i < n-1; i++) {
		// 		x = a[i].length();
		// 		for (int j = i+1; j < n; j++) {
		// 			y = a[j].length();
		// 			if (x <= y && a[j].startsWith(a[i]) && a[j].endsWith(a[i]))
		// 				r++;
		// 		}
		// 	}
		// 	return r;
		...
		// // 亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！
		// // 感觉这个题目出得好愚钝呀，亲爱的表哥的活宝妹，就得慢慢去瓣。。。这种题出得太没意思了，写得也太没意思了。。。。
		// static int [][] dirs = {{1, 0}, {0, 1}, {0, -1}, {-1, 0},
		// 				 {1, 1}, {-1, 1}, {1, -1}, {-1, -1}};
		// public int mostFrequentPrime(int[][] a) {
		// 	m = a.length; n = a[0].length; this.a = a; 
		// 	f = new HashMap<>(); // for frequency counting
		// 	vis = new boolean [m][n];
		// 	for (int i = 0; i < m; i++) 
		// 		for (int j = 0; j < n; j++) {
		// 			vis[i][j] = true;
		...
		// 	int maxCnts = Collections.max(f.values());
		// 	// System.out.println("f.size(): " + f.size());
		// 	// for (Map.Entry<Integer, Integer> en : f.entrySet()) 
		// 	// 	System.out.print(en.getKey() + ", " + en.getValue() + "\n");
		// 	for (int key : f.keySet()) {
		// 		if (f.get(key) == maxCnts && (ans == -1 || key > ans))
		// 			ans = key;
		// 	}
		// 	return ans;
		// }
		// Map<Integer, Integer> f; // for frequency counting
		// boolean [][] vis;
		// int [][] a;
		// int m, n;
		...
		// // 亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！
		// // 亲爱的表哥的活宝妹，不知道怎么写这个题目，先放一下：不知道这个题目怎么想的？！！
		// public int longestCommonPrefix(int[] a, int[] b) {
		// 	int m = a.length, n = b.length;
		// 	TreeSet<Integer> ta = new TreeSet<>((x, y)->y-x); // 降序
		// 	TreeSet<Integer> tb = new TreeSet<>();
		// 	for (int v : a) ta.add(v);
		// 	for (int v : b) tb.add(v);
		// 	for (int v : ta) {
		// 		Integer hi = tb.ceiling(v);
		// 		if (hi != null && hi == v)
		// 	}
		// }

		// // 亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！
		// public long countPrefixSuffixPairs(String[] a) {
		// 	int n = a.length;
		// }
		...
		// // 亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！
		// // 【左下角、升序排序】：右上角，要按左下角的顺序排序。先写其它的，烦死了这种恶心题目		
		// public long largestSquareArea(int[][] a, int[][] b) {
		// 	int n = a.length;
		// 	List<int []> l = new ArrayList<>();
		// 	for (int i = 0; i < n; i++)
		// 		l.add(List.of(a[i][0], a[i][1], b[i][0], b[i][1]));
		// 	Collections.sort(l, (x, y)->x[0] != y[0] ? x[0]-y[0] : x[1] - y[1]);
		// 	// 遍历：找出所有、有交叉，相邻2 个长方体间的最大可能正方形边长
		// 	int [] p = l.get(0), c; // prev rect; curr rect
		// 	for (int i = 1; i < n; i++) {
		// 		// 跟前一个长方形：无交叉
		// 		c = l.get(i);
		// 		// 只比较前一个，在上边下边情况下，可能会漏掉解
		// 		if (c[0] >= p[2] || c[1] <= p[1] || c[1] >= p[3]) { // 右边、上边、下边
		// 			p = c;
		// 			continue;
		// 		}
		// 		// 跟前一个长方形：有交叉. 它可能不止与前一个有交叉，它可能与前面N 个有交叉，找一个最大交叉边
		// 	}
		// }

		// // 亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！
		// // 没读懂：不知道它在说的是什么意思
		// // 用个【二分查找】从【sum, m】范围内，二分找一个最小解；每个最小解检测 boolean-isPossibleToSucceed() 
		// public int earliestSecondToMarkIndices(int[] a, int[] b) { // 556 / 570 testcases passed 不知道哪里还有点儿细节错了。。。
		// 	m = b.length;			n = a.length;
		// int sum = 0;
		// 	for (int v : a) sum += v;
		// 	if (sum >= m) return -1;
		// 	int l = sum, r = m, ans = Integer.MAX_VALUE;
		// 	while (l <= r) {
		// 		int mid = l + (r - l) / 2;
		// 		if (isPossibleToSucceed(mid, a, b)) {
		...
		// }
		// int m, n;
		// boolean isPossibleToSucceed(int v, int [] a, int [] b) {
		// 	Map<Integer, Integer> map = new HashMap<>();
		// 	for (int i = v-1; i >= 0; i--) {
		// 		int j = b[i]-1; // 对应 a 的下标
		// 		if (!map.containsKey(j)) map.put(j, i);
		// 	}
		// 	if (map.size() < n) return false;
		// 	// 接下来，就是数呀：数 v 秒钟，够不够用，把它们变成为 0. 这里面还有点小小技巧机关什么的
		// 	// 倒着数: 倒着检查，最后一个出现的数，够不够用
		// 	int sum = 0;
		// 	int markedCnts = 0;
		// 	for (int i = v-1; i >= 0; i--) {
		// 		if (map.get(b[i]-1) != i) continue; // 不是最后一次出现的，可以不管
		// 		if (v - sum - markedCnts > a[b[i]-1]) {
		// 			sum += a[b[i]-1];
		// 			markedCnts++;
		// 		} else return false;
		// 	}
		...
		// 	int n = a.length, j = 0, k = 0;
		// 	int [] l = new int [n], r = new int [n];
		// 	l[j++] = a[0];
		// 	r[k++] = a[1];
		// 	for (int i = 2; i < n; i++)
		// 		if (l[j-1] > r[k-1]) l[j++] = a[i];
		// 		else r[k++] = a[i];
		// 	k = 0;
		// 	while (j < n) l[j++] = r[k++];
		// 	return l;
		...
		// 	// 亲爱的表哥的活宝妹，忘记这个地方，是要怎么向右下拉伸、动规算二维的累积和的个数了，有个什么:【数组上】二维动规，转化为一维动规的？
		// 	// 试：遍历列；对每列，二维遍历行
		// 	for (int j = 1; j <= n; j++) {
		// 		for (int i = 1; i <= m; i++) {
		// 			if (f[i][j] <= k)
	...
	// 		// Y0: 0 1 2
	// 		// N1: 0 1 2
	// 		return n*n - Math.max(r[0][0] + Math.max(r[1][1], r[1][2]), Math.max(r[0][1]+Math.max(r[1][0], r[1][2]), r[0][2] + Math.max(r[1][0], r[1][1])));
	// 	}
...
// 			int n = a.length, j = 0, k = 0;
// 			int [] l = new int [n], r = new int [n];
// 			TreeMap<Integer, Integer> ml = new TreeMap<>(), mr = new TreeMap<>();
// 			l[j++] = a[0]; r[k++] = a[1];
// 			ml.put(a[0], 1); mr.put(a[1], 1);
// // 需要拆解： greaterCnt(int 【】 a,int val) 函数。感觉忘记这个题目了，暂时没有思路，今天不写了。。
// 		}

		// // 亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！
		// public int minimumBoxes(int[] a, int[] b) {
		// 	int n = a.length, m = b.length, r = 0, sum = Arrays.stream(a).sum();
		// 	Arrays.sort(b);
