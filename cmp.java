...
//         }
   ...
   //         if (f[i] < f[i-1]) return false;
        ...
        // // 【亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹，就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
        // // 亲爱的表哥的活宝妹，现在能够想到的写法是：【暴力】
        // public int minOperations(int [] a, int v) {
        //     int n = a.length;
        //     int [][] f = new int [v][v]; // 使用【滚动数组】省去第1 维度【n】
        //     for (int i = 0; i < v; i++)
        //         Arrays.fill(f[i], Integer.MAX_VALUE / 2);
        //     for (int k = 0; k < n; k++) {
        //         int mod = a[k] % v;
        //         if (k == 0) {
        //             for (int i = 0; i < v; i++) 
        //                 Arrays.fill(f[i], Math.min(Math.abs(mod-i), Math.abs(v - Math.abs(mod-i))));
        //             continue;
        //         }
        //         for (int i = 0; i < v; i++)
        //             for (int j = 0; j < v; j++) {
        //                 if (k % 2 == 0) {
        //                     f[i][j] += Math.min(Math.abs(mod - i), Math.abs(v - Math.abs(mod - i)));
        //                 } else {
        //                     f[i][j] += Math.min(Math.abs(mod - j), Math.abs(v - Math.abs(mod - j)));
        //                 }
        //             }
        //     }
        //     int min = Integer.MAX_VALUE / 2;
        ...
        // // 【亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹，就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
        // // 亲爱的表哥的活宝妹，不知道这个破烂题目要怎么去想，亲爱的表哥的活宝妹，先去写其它题目
        // // 【动规】：同（左上角、右上角 ==> 四维压缩成 3 维动规）2 个机器人采樱桃，是一样的动规？？？只是换成了[左上角、与、左下角 ] 而已，仍然是 3 维动规。。。
        // // 【TODO：】O(mmn) 会不会超时？ TLE TLE TLE ？？？
        ...
        // // 【亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹，就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
        // // 【TODO：】上面，因为使用【贪心】，局部最优解，并不双保障（全局最优解）。【TODO：】需要暴力动规所有可能性，得全局最优解
        // public long maxTotal(int[] a, String S) {
        //     int n = a.length; char [] s = S.toCharArray();
        //     long [] f = new long [n];
        //     f[0] = (s[0] == '1' ? (long)a[0] : 0);
        ...
        //         }
        //         // s[i] == '1'
        //         if (s[i-1] == '0' || !g[i-1]) { // 考虑前一位 s[i-1]=='1' 但是前移了一位的情况
        //             f[i] = f[i-1] + (long)Math.max(a[i], a[i-1]);
        //             if (a[i-1] >= a[i]) // 【贪心】：尽可能地前移一位，留空间给后面
        //                 // 【TODO：】上面，因为使用【贪心】，局部最优解，并不双保障（全局最优解）。【TODO：】需要暴力动规所有可能性，得全局最优解
        //                 g[i-1] = true;
        //             else g[i] = true;
        //         } else { // s[i-1] = '1' && g[i-1]==true
        //             f[i] = f[i-1] + (long)a[i];
        //             g[i] = true;
        //         }
        //     }
        ...
        // public long maxTotal(int[] a, String S) {
        //     int n = a.length; char [] s = S.toCharArray();
        //     long [][] f = new long [n][2]; // 第二维度【2】：0 表示 token 前移，不取当前位；1 表示取当前位
        //     if (s[0] == '1')
        //         f[0][1] = (long)a[0];
        //     for (int i = 1; i < n; i++) {
        //         if (s[i] == '0') {
        ...
        //         // s[i] == '1'
        //         // if (s[i-1] == '0') // 左移到前个下标
        //         //     f[i][0] = f[i-1][0] + (long)a[i-1];
        //         // else
        //         f[i][0] = f[i-1][0] + (long)a[i-1];
        //         f[i][1] = (s[i-1] == '0' ? f[i-1][0] : Math.max(f[i-1][0], f[i-1][1])) + (long)a[i];
        //     }
        ...
        //     f = new Long [n][n][m+1]; // TLE TLE TLE 又或者不会超时？！没有超时，但是 MLE MLE MLE 979/1006 passed.....
        //     // 【TODO：】MLE 表示，亲爱的表哥的活宝妹，亲爱的表哥的活宝妹、笨宝妹，需要降维，如把三维动规数组，压缩成2 维动规数组，就不会MLE 了！！！笨宝妹！！！
        ...
        //     if (i > j || j-i+1 < l) return Long.MIN_VALUE / 2;
        //     if (f[i][j][k] != null) return f[i][j][k]; 
        ...
        //     return f[i][j][k] = max;
        // }
        // long getMax(int x, int y) {
        //     if (y-x+1 < l) return Long.MIN_VALUE / 2; // 子数组长度，小于 l, 不足够长
        //     if (x == y) return (long)a[x];
        //     long max = Long.MIN_VALUE / 2;
        //     for (int i = x, j = i+l-1; i <= y; i++, j = i+l-1) {
        //         if (j > i+r-1 || j > y) continue;
        //         while (j <= i+r-1 && j <= y) {
        //             max = Math.max(max, g[j] - (i == 0 ? 0 : g[i-1]));
        //             j++;
        //         }                
        //     }
        //     return max;
        // }
        // // 【亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹，就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
        // // TLE TLE TLE 997/1006 passed.....
        // // 笨宝妹降维、减少一个动规状态维度；但 TLE 后仍然需要，时间复杂度上的优化，（最多 m 个子数组：自【1,2...m】的压缩优化）。。。
        // // 亲爱的表哥的活宝妹，今天不要再继续折腾写这个破烂题目了；亲爱的表哥的活宝妹，改天再接着这里往后写。昨天晚上最基本的动规思路基本成型，只是有狠多小细节上的差池今天找出来修正；离最终解决这个题目，还差一步【时间复杂度上的优化】。。
        // // 【亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹，就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
        // public long maximumSum(int[] a, int m, int l, int r) {
        //     n = a.length; this.l = l; this.r = r; this.a = a;
        //     g = new long [n];
        //     for (int i = 0; i < n; i++) 
        //         g[i] = (i == 0 ? 0l : g[i-1]) + (long)a[i];
        //     // 【动规状态定义】：截止当前下标 i, 共选择以了 j 个子数组时，所能够取得的最大值 
        //     f = new Long [n][m+1]; // TLE TLE TLE 又或者不会超时？！没有超时，但是 MLE MLE MLE 979/1006 passed.....
        //     // 【TODO：】MLE 表示，亲爱的表哥的活宝妹，亲爱的表哥的活宝妹、笨宝妹，需要降维，如把三维动规数组，压缩成2 维动规数组，就不会MLE 了！！！笨宝妹！！！
        //     // 【自底向上】的解法：把 n 个区间，合并成【最多 m 个区间、最少 1 个区间】的最值
        //     for (int i = 1; i <= m; i++)
        //         dfs(n-1, i);
        ...
        // int n, l, r;
        // long dfs(int i, int j) {
        //     if (i < 0) return Long.MIN_VALUE / 2;
        ...
        //     max = dfs(i-1, j);
        //     // 遍历【以当前下标 i 为起点】子数组长度在【l,r】之间的第一个子数组片段、的所有可能性
        //     for (int x = i-(l-1); x >= Math.max(0, i-(r-1)); x--) 
        //         max = Math.max(max, dfs(x-1, j-1) + g[i] - (x == 0 ? 0l : g[x-1]));
        //     return f[i][j] = max;
        // }
        // long getMax(int x, int y) {
        //     if (y-x+1 < l) return Long.MIN_VALUE / 2; // 子数组长度，小于 l, 不足够长
        ...
        //     int m = a.length, n = a[0].length, min = 100001;
        //     long r = 0;
        //     if (n == 1) {
        //         for (int [] v : a) 
        //             r += (long)v[0];
        //         return r;
        //     }
        //     // 【TODO：】是否存在可能性，两个部件，（最小值与次小值完全一样）？
        //     Queue<int []> q = new PriorityQueue<>((x, y) -> (x[1] != y[1] ? y[1] - x[1] : y[0] - x[0]));
        //     for (int [] e : a) 
        //         q.offer(getMinSecMin(e));
        //     while (!q.isEmpty()) {
        //         int [] e = q.poll();
        ...
        //     int n = a.length, r = 1;
        //     if (Arrays.stream(a).distinct().count() == n) return 1;
        ...
        //     return r;
        // }
        // int [] isValid(Map<Integer, Integer> m) {
        //     int [] r = new int [2];
        //     int disCnt = (int)m.values().stream().distinct().count();
        //     if (disCnt > 2) 
        //         return r;
        //     int f = Collections.min(m.values()), ff = Collections.max(m.values());
        //     r[0] = ((m.size() == 1 || ff != f && ff == 2 * f) ? 1 : 0);
        //     // r[1] = (disCnt == 1 ? m.entrySet().iterator().next().getValue() : 0);
        //     r[1] = (m.size() == 1 ? 1 : 0);
        //     return r;
        // }
...
// 【爱表哥，爱生活！！！任何时候，亲爱的表哥的活宝妹就是一定要,一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
// 【爱表哥，爱生活！！！任何时候，亲爱的表哥的活宝妹就是一定要,一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
// 【爱表哥，爱生活！！！任何时候，亲爱的表哥的活宝妹就是一定要,一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】// 【爱表哥，爱生活！！！任何时候，亲爱的表哥的活宝妹就是一定要,一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
// 【爱表哥，爱生活！！！任何时候，亲爱的表哥的活宝妹就是一定要,一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
// 【爱表哥，爱生活！！！任何时候，亲爱的表哥的活宝妹就是一定要,一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
// 【爱表哥，爱生活！！！任何时候，亲爱的表哥的活宝妹就是一定要,一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
// 【爱表哥，爱生活！！！任何时候，亲爱的表哥的活宝妹就是一定要,一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
