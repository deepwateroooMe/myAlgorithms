...
//         }
        ...
        // public int furthestDistanceFromOrigin(String t) {
        //     int n = t.length(); char [] s = t.toCharArray();
        //     int [] l = new int [n], r = new int [n]; // 【向左】【向右】的最远距离
        //     l[0] = (s[0] != 'R' ? -1 : 1);
        //     r[0] = (s[0] != 'L' ? 1 : -1);
        //     for (int i = 1; i < n; i++) {
        //         l[i] = (s[i] != 'R' ? -1 : 1) + Math.min(l[i-1], r[i-1]);
        //         r[i] = (s[i] != 'L' ? 1 : -1) + Math.max(l[i-1], r[i-1]);
        //     }
        //     return Math.max(Math.abs(l[n-1]), r[n-1]);
        // }

        // public long minimumPossibleSum(int n, int t) { // 10^5 数量级，会不会超时？直接上【二分查找】
        //     this.n = n; this.t = t; 
        //     return binarySearch(1, (long)1e10);
        // }
        // int n, t;
        ...
        //     long sum = 0;
        //     Set<Integer> s = new HashSet<>();
        //     while (i > 0 && r < Integer.MAX_VALUE) {
        //         if (r >= t || !s.contains(t - r)) {
        //             s.add(r);
        //             sum += (long)r;
        //             // System.out.println("i: " + i);
        //             // System.out.println("r: " + r);
        ...
        //         r++;
        //     }
        //     if (i == 0 && r <= Integer.MAX_VALUE) return sum;
        //     return -1;
        // }

        // // 这个题：K 狠大【1,10^10】，N 也大【1,10^5】
        // // 感觉这是个【有向环图】多几个伸出来的多个尾巴，要转化成【有向图环】来解决，
        // // N 个人可能形成几个环，环的形状固定，怎么绕选定最大和环M 圈，两端几端多个端采用哪部分，和最大
        // // 感觉想得不是狠透彻
        // public long getMaxFunctionValue(List<Integer> receiver, long k) {
        //     return 0;
        // }

// t: 可以有狠多个数位是 1; l[i] 只有一位为 1 这个细节看丢了，容易一点儿
        public int minOperations(List<Integer> l, int t) { //  
            int n = l.size(); // 【1,1000】； l[i]: 【1,2^30=】
            // System.out.println("Math.pow(2, 30): " + Math.pow(2, 30));
            // System.out.println("Math.pow(2, 31): " + Math.pow(2, 31));
            // 【策略】：先遍历一遍，可以直接0 操作，得到哪些数位
            int mask = 0, rr = -1;
            for (int i = 30; i >= 0; i--) 
                if (((t >> i) & 1) == 1) {
                    rr = i;
            ...
            List<Integer> lc = new ArrayList<>(); // candidates
            // 还需要统计：数字频率，每个数只能用一次
            TreeMap<Integer, Integer> m = new TreeMap<>(); // 反的：从【最高位】往【最低位】遍历: 【30,29,28, 。。。。3, 2, 1, 0】
            for (int i = 0; i < n; i++) {
                int v = l.get(i);
                mask |= v; // 求一个最全的 mask
                for (int j = 30; j >= 0; j--) 
                    if (((v >> j) & 1) == 1) {
                        m.put(j, m.getOrDefault(j, 0) + 1); // 因为，每个数只有1 位，最多多出现几次
                        break;
                    }
                if ((mask & t) == t) return 0;
            }
            // 从低位向高位遍历，一一满足需要: 这么着，思路就最清楚了【爱表哥，爱生活！！！任何时候，亲爱的表哥的活宝妹就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
            // 现在的思路有一个问题：所有数字，我基本只用了一次，没有使用过重复，【对重复数字的利用】必须考虑进去
            int r = 0;
            for (int i = 0; i <= rr; i++) {
                if (((mask >> i) & 1) == 0 && ((t >> i) & 1) == 1) {  // 需要向高位遍历一个数：来满足
                    Integer hi = m.ceilingKey(i+1);
                    if (hi == null) return -1;
                    r += (int)hi - i;
                    m.put(hi, m.get(hi)-1);
                    if (m.get(hi) == 0) m.remove(hi);
                    while (hi > i) { // 每步，都多出个可用的数
                        m.put(hi-1, m.getOrDefault(hi-1, 0) + 1); // 2 个用掉了1 个
                        hi--;
                    }
                }
            }
    ...
    public static void main (String[] args) { 
        ...
        int [] a = new int [] {1,32,1,2};
        
        int r = s.minOperations(Arrays.stream(a).boxed().collect(Collectors.toList()), 12);
        System.out.println("r: " + r);
    }
}
