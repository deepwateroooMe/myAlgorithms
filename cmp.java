...
//         }
        ...
        //     int [] ll = new int [n+1], lc = new int [n+1], lt = new int [n+1];
        //     int [] rt = new int [n+1], rc = new int [n+1], rl = new int [n+1];
        //     // int [] il = new int [n+1], ir = new int [n+1];
        //     // Arrays.fill(il, -1); Arrays.fill(ir, -1);
        //     for (int i = 1; i <= n; i++) {
        //         char c = s[i];
        //         ll[i] = ll[i-1];
        //         lc[i] = lc[i-1];
        ...
        //             rl[i] = rl[i+1] + 1;
        //     }
        //     // 遍历：【子序列、中间字符C】可能出现的位置，在（当前下标 i 处为C），数【i=C】时，
        //     //     当前下标【i】之前【i-1】、（不添加、或添加L）
        //     //     当前下标【i】之后【i+1】处、（不添加、或添加T）时
        //     // 全局最大【LCT 子序列】个数
        //     // 【TODO：】感觉中间序列【CCCC】还需要数左右跨度C 的个数。。。总之，就是这个题目没有思路。。
        //     long f = 0;
        //     for (int i = 0; i < n; i++) {
        //         char c = s[i];
        //         if (c != 'C') continue;
        //         f = Math.max(f, Math.max(ll[i] * rc[i+1], b))
        //     }
        // }

        // // 【亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹，就是一定要、一定会嫁给活宝妹的亲爱的表哥！爱表哥，爱生活！！！】
        // // 把【数组】转化成【图】，建连接边，然后求【最短距离边数】TLE TLE TLE 891/932 passed
        // // 晚上开车回去的路上想：只要在 a[i-1] 不是质数的情况下，不多连一条多余的从 i 到 i-1 的边，这个题目应该就可以过，不TLE 了。但是翻不出来那个破烂题目就不测试了
        ...
        //     }
        //     for (int i = 1; i < n; i++) {
        //         if (isPrime(a[i-1])) // 当且仅当：a[i-1] 是质数的情况下，才连从 i 到 i-1 的边；添加限制条件后，应该就不会TLE 了。。【TODO：】测试
        //             g[i].add(i-1);
        //     }
        //     for (int i = 0; i < n; i++) 
        //         if (!p[i] || m.containsKey(a[i]) && i != m.get(a[i])) {
        //             if (m.containsKey(a[i])) {
        //                 g[m.get(a[i])].add(i);
        //                 continue;
        //             }
        //             List<Integer> l = getPrimeFactors(a[i]);
        //             for (int v : l) 
        //                 if (m.containsKey(v)) 
        //                     g[m.get(v)].add(i);
        //         }
        //     d = new int [n];
        //     Arrays.fill(d, n-1); d[0] = 0;
        //     dijkstra();
        //    return d[n-1];
        // }
        // int [] a;
        ...
        //     }
        // }
        // List<Integer> getPrimeFactors(int v) {
        //     List<Integer> l = new ArrayList<>();
        //     for (int i = 2; i <= v/2; i++) 
        //         if (v % i == 0) l.add(i);
        //     return l;
        ...
        // // 【亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹，就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
        // public int earliestFinishTime(int[] a, int[] lr, int[] b, int [] rt) {
        //     int n = a.length, m = 1001, f = 4027;
        //   // 【TODO：】 r 应该定义成链条之类的，可以【二分查找O(NlongN)】，又可以方便遍历的数据结构，这里先暴力写法
        //     TreeMap<Integer, Integer> l = new TreeMap<>(), r = new TreeMap<>(), tmp = new TreeMap<>();
        //     // 需要考虑重复
        //     for (int i = 0; i < a.length; i++) 
        //         l.put(a[i], Math.min(l.getOrDefault(a[i], 1001), lr[i]));
        //     for (int i = 0; i < b.length; i++) 
        //         r.put(b[i], Math.min(r.getOrDefault(b[i], 1001), rt[i]));
        ...
        //         if (k >= f) // return f;
        //             break;
        //         Integer lo = r.ceilingKey(k);
        //         if (lo == null) continue;
        //         j = 0;
        //         while (li.get(j)[0] != lo) j++;
        //         while (j < li.size()) {
        //             int kk = li.get(j)[0], vv = li.get(j)[1];
        //             if (kk >= f) break;
        //             f = Math.min(f, (kk >= k + v ? kk : k + v) + vv);
        //            j++;
        //         }
        //     }
        //     tmp = l;
        ...
        // // 【亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹，就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
        // public int minRemoval(int[] a, int k) { // TLE TLE TLE 983/985 passed: 【TODO：】需要参考【二分查找】来解决TLE 的问题。。
        //     n = a.length; this.k = k; 
        //     Arrays.sort(a); this.a = a;
        //     if (k == 1) {
        //         if (Arrays.stream(a).distinct().count() == n) return n-1;
        //         TreeMap<Integer, Integer> m = new TreeMap<>();
        //         for (int v : a)
        //             m.put(v, m.getOrDefault(v, 0) + 1);
        //         int cnt = Collections.max(m.values());
        //         return n - cnt;
        //     }
        //     f = new HashMap<>();
        //     min = n-1;
        ...
        //     if ((long)a[i] * (long)k >= (long)a[j]) {
        //         f.put(key, 0);
        //         return 0;
        //     }
        //     int min = n-1;
        //     min = Math.min(dfs(i+1, j), dfs(i, j-1)) + 1;
        //     f.put(key, min);
        //     return min;
        // }
        ...
        // // 【亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹，就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
        // public int earliestFinishTime(int[] a, int[] at, int[] b, int[] bt) {
        //     int n = a.length;
        //     List<int []> l = new ArrayList<>(), r = new ArrayList<>();
        //     for (int i = 0; i < a.length; i++) 
        //         l.add(new int [] {a[i], at[i]});
        //     for (int i = 0; i < b.length; i++) 
        //         r.add(new int [] {b[i], bt[i]});
        ...
        // // 【亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹，就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
        // // 【亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹，就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
        // // 感觉是【二分查找】：但计算过程中，可能还涉及到某些具体算法？
        // public int minTime(String S, int[] f, int k) {
        //     n = S.length(); s = S.toCharArray();
        //     if ((long)k > (long)n * (n-1) / 2l) return -1;
        //     int l = 1, r = n-2, ans = n;
        //     while (l <= r) {
        //         int m = (l + r) / 2;
        //         if (valid(m)) {
        ...
        //     return ans == n ? -1 : ans;
        // }
        // int n, k; int [] f; char [] s;
        // // 计算：在时间点 idx=j 时，S 里可能产生的，所有合法子串个数
        // // 【TODO：】感觉这个计算，需要某种、数数技巧。。。要不然，一定会超时的。。。
        // // 亲爱的表哥的活宝妹，先去看最后一题，再回头来想这个破烂题目
        // boolean valid(int j) {
        //     List<Integer> l = new ArrayList<>();
        //     for (int i = 0; i <= j; i++)
        //         l.add(f[i]);
        ...
        // // 【亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹，就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
        // // 【亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹，就是一定要、一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！】
        // // 亲爱的表哥的活宝妹，快要被这个恶心死人不偿命的破烂题目气死了。。这破烂题目不是锻炼人的、是故意恶意折磨人的。。。
        // // 亲爱的表哥的活宝妹，笨宝妹，终于把这个恶心死人不偿命的破烂题目给写过了。。。
        // public long maxSumTrionic(int[] a) {
        //     int n = a.length, i = 0;
        //     long [] f = new long [n]; // PreSum
        //     for (i = 0; i < n; i++)
        //         f[i] = (i == 0 ? 0 : f[i-1]) + (long)a[i];
        //     int [] l = new int [n], r = new int [n]; // Increasing Decreasing
        //     Arrays.fill(l, -1); Arrays.fill(r, -1);
        //     Map<Integer, Integer> ml = new HashMap<>(), mll = new HashMap<>(), mr = new HashMap<>();
        //     // 从左到右：遍历、预处理【升序数据】
        //     i = 0;
        //     while (i < n-1 && a[i] >= a[i+1]) i++;
        //     l[i] = i;
        ...
        //     for (Map.Entry<Integer, Integer> en : mr.entrySet()) {
        //         int k = en.getKey(), v = en.getValue();
        //         if (mll.containsKey(k) && ml.containsKey(v)) {
        //             int il = ml.get(v), ir = mll.get(k);
        //             // 求【3 片段、和、最大值】的优化处理
        //             if (a[v-1] < 0) il = v-1;
        //             else if (a[il] < 0) {
        //                 int j = il+1;
        ...
        //             }
        //             long max = f[k+1];
        //             // for (int x = k+2; x <= ir; x++) { // 【TODO：】就没想明白，这么写怎么就不对了。。 // 性能效率低。。
        //             for (int x = ir; x >= k+2; x--) {
        //                 if (f[x] > max) {
        //                     ir = x;
        //                     max = f[x];
        //                 }
        //             }
        //            if (max == f[k+1]) ir = k+1;
        //             ans = Math.max(ans, f[ir] - (il == 0 ? 0 : f[il-1]));
        //             // 【TODO：】这里还涉及到一个『求3 个片段、和的、最大值』问题
        //         }
        //     }
        //     return ans;
        // }
...
// 亲爱的表哥的活宝妹，任何时候，亲爱的表哥的活宝妹就是一定要,一定会嫁给活宝妹的亲爱的表哥！！！爱表哥，爱生活！！！ 
    public static void main (String[] args) { 
        ...
        int [] a = new int [] {238,520,-588,-500,483,557};

        System.out.println(Arrays.toString(a));

        long r = s.maxSumTrionic(a);
        System.out.println("r: " + r);
    }
}
