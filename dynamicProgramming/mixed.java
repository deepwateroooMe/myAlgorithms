...
// TLE TLE TLE AT 16/81 【这里需要搞明白】：用栈呀什么的，只是借助数据结构，帮助自己的处理和数的过程，你也可以不用数据结构直接用数的呀，直接用数的就转成 dp 了呀？！！！
        public boolean hasValidPath(char[][] a) { 
            if (a[0][0] == ')') return false;
            m = a.length;
            n = a[0].length;
            vis = new boolean [m][n];
            return dfs(0, 0, new Stack<Character>(), a);
        }
        int [][] dirs = {{1, 0}, {0, 1}};
        boolean [][] vis;
        int m, n;
        boolean dfs(int i, int j, Stack<Character> s, char [][] a) {
            if (i < 0 || i >= m || j < 0 || j >= n || vis[i][j] || s.isEmpty() && a[i][j] == ')') return false;
            vis[i][j] = true;
            if (s.isEmpty() || a[i][j] == '(') s.push(a[i][j]); // (
            if (!s.isEmpty() && s.peek() == '(' && a[i][j] == ')') s.pop();
            if (i == m-1 && j == n-1 && s.isEmpty()) return true;
            for (int [] d : dirs) {
                int x = i + d[0], y = j + d[1];
                if (dfs(x, y, s, a)) return true;
            }
            vis[i][j] = false;
            if (a[i][j] == '(') s.pop();
            else s.push('(');
        ...
        public boolean hasValidPath(char[][] a) {
            ...
            // dp[i][j][k] := 1 if there's a path from grid[i][j] to grid[m - 1][n - 1]
            // With [# of '('] - [# of ')'] == k
            f = new Integer [m][n][m+n];
            return hasValidPath(0, 0, 0, a) == 1 ? true : false;
        }
        Integer [][][] f;
        int m, n;
        int hasValidPath(int i, int j, int k, char [][] a) { // 这，不是， DFS 记忆化搜索吗？
            if (i == m || j == n) return 0;
            k += a[i][j] == '(' ? 1 : -1; // K: 这里的作用，就是数，剩余左边括号（不配对的个数【这里动态更新了 k 的值】
            if (k < 0) return 0;
            if (i == m-1 && j == n-1) return k == 0 ? 1 : 0;
            if (f[i][j][k] != null) return f[i][j][k];
            return f[i][j][k] = hasValidPath(i+1, j, k, a) | hasValidPath(i, j+1, k, a);
        }
        ...
        public int divisorSubstrings(int num, int k) {
            ...
            int n = t.length(), r = 0;
            for (int i = 0; i+k <= n; i++) {
                int v = Integer.parseInt(t.substring(i, i+k));
                if (v != 0 && num % v == 0) r++;
            }
            return r;
        }
        ...
        public int waysToSplitArray(int[] a) {
            ...
            for (int i = 0; i < n; i++) 
                l[i] = (i == 0 ? 0l : l[i-1]) + (long)a[i];
            for (int i = n-1; i >= 0; i--) 
                r[i] = (i == n-1 ? 0l : r[i+1]) + (long)a[i];
            for (int i = 0; i < n-1; i++) 
                if (l[i] >= r[i+1]) cnt++;
            return cnt;
        }
    ...
    public static void main (String[] args) {
        ...
        int [][] a = new int [][] {{1,5},{10,11},{12,18},{20,25},{30,32}};

        int r = s.maximumWhiteTiles(a, 10);
