...
public class hdseven {
    public static class Solution {
        ...
        public boolean isSolvable(String[] sa, String res) { // 权重累加：裁枝优化
            for (String s : sa) {
                int w = 1, idx;
                for (int i = s.length()-1; i >= 0; i--) {
                    idx = s.charAt(i) - 'A';
    ...
    // https://leetcode-cn.com/problems/building-boxes/solution/zui-qing-xi-de-ti-jie-copywai-guo-you-re-pnlv/
        public int minimumBoxes(int n) { // 1739
            int cur = 0, i = 0, j = 0;
            while (cur < n) {
                ++j;
                i += j;
    ...
    // 根据题意需要找到前k大的数，又需要求区间和，就自然想到线段树.写起来比较不容易出错。
    // 维护2个线段树数组，一个记录各数值出现的个数，一个记录区间值——累积和？，
    ...
    class BST { // todo: 改天把这个写完
        class TreeNode {
            public int val, sum, cnt, size;
    ...
    public int calculateMKAverage() {
        ...
        // hdseven s = new hdseven(6, 1); 
        // s.addElement(3);        // current elements are [3]
        // s.addElement(1);        // current elements are [3,1]
        // int r = s.calculateMKAverage(); // return -1, because m = 3 and only 2 elements exist.
        // System.out.println("r: " + r);
        // s.addElement(12);       // current elements are [3,1,10]
        // int r2 = s.calculateMKAverage(); // The last 3 elements are [3,1,10].
        // System.out.println("r2: " + r2);
        // // After removing smallest and largest 1 element the container will be [3].
        // // The average of [3] equals 3/1 = 3, return 3
        // s.addElement(5);        // current elements are [3,1,10,5]
        // s.addElement(3);        // current elements are [3,1,10,5,5]
        // s.addElement(4);        // current elements are [3,1,10,5,5,5]
        // int r3 = s.calculateMKAverage(); // The last 3 elements are [5,5,5].
        // System.out.println("r3: " + r3);
        // After removing smallest and largest 1 element the container will be [5].
        // The average of [5] equals 5/1 = 5, return 5
        // int r = s.minimumBoxes(10);
        // System.out.println("r: " + r);
    int m, k, n;
    Queue<Integer> que = new LinkedList<>();//数据流中的最后m个元素
    PriorityQueue<Integer> minHeap, minHeapNums;//最大的k个数、除最小的k个数之外的数
